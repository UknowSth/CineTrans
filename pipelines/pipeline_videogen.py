...
class VideoGenPipeline(DiffusionPipeline):
    ...
    def encode_prompt(
        ...
        if self.text_encoder is not None:
            ...
            self.text_encoder = self.vitL_encoder 
            self.tokenizer = self.long_tokenizer
            # print('switch to long clip!')  ###########
        ...
        if self.text_encoder is not None:
            self.text_encoder = old_text_encoder
    ...
    def __call__(
        ...
        shot_frames = prompt['shot_info']
        general_caption = prompt['general_caption']

        assert sum(shot_frames) == video_length, "video length error."

        shot_info = np.concatenate([np.repeat(num, size) for num, size in enumerate(shot_frames)])
        # shot_index = list(whole_shot_index[frame_indice])

        general_prompt_embeds = self.encode_prompt(
        # general_prompt_embeds = self._encode_prompt(
            general_caption,
            device,
            num_images_per_prompt,
            do_classifier_free_guidance,
        ...
        latents = self.prepare_latents(
            ...
            height,
            width,
            general_prompt_embeds.dtype,
            device,
            generator,
            latents,
        )

        # random_index = torch.Tensor([4, 13, 8, 15, 6, 14, 2, 1, 11, 5, 12, 3, 7, 9, 10, 0]).to(dtype=torch.int64)
        # latents = latents[:, :, random_index, ...]
        ...
        shot_info = torch.tensor(shot_info).unsqueeze(0).to(device)
        shot_info = torch.cat([shot_info] * 2) if do_classifier_free_guidance else shot_info
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.unet(
                    latent_model_input,
                    t,
                    encoder_hidden_states=general_prompt_embeds,
                    shot_info=shot_info,
