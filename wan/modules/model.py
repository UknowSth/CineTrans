...
class WanSelfAttention(nn.Module):
    ...
    def forward(self, x, seq_lens, grid_sizes, freqs, attn_mask, visual=False):
        r"""
        Args:
...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        # print(f"++++++++++++++ context.shape:{context.shape}") # b, 1024, 1536 第二维度表示序列长度
        ...
        for ii,block in enumerate(self.blocks):
            if ii >= self.start_masked_layer and ii <= self.end_masked_layer:
                ...
                if self.training and use_gradient_checkpointing:
                    ...
                    x = block(x, e0, seq_lens, grid_sizes, self.freqs, context, context_lens, None, None, visual)
