# Temporal attention 
# Only surpport AttnProcessor, AttnProcessor2_0 at this moment
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        assert isinstance(processor, AttnProcessor2_0) or isinstance(processor, AttnProcessor)

        self.rotary_emb = rotary_emb

    # temporal attention does not surpport xformers at this moments by xin
    ...
    def separate_heads(self, tensor):
        batch_size, seq_len, dim = tensor.shape
        head_size = self.heads
        tensor = tensor.reshape(batch_size, seq_len, head_size, dim // head_size)
        # tensor = tensor.permute(0, 2, 1, 3).reshape(batch_size * head_size, seq_len, dim // head_size)
...
class AttnProcessor:
    ...
    def __call__(
        ...
        query = attn.separate_heads(query)
        query = rearrange(query, 'b s h d -> b h s d').contiguous()
        query[:, :, :video_length, :] = attn.rotary_emb.rotate_queries_or_keys(query[:, :, :video_length, :])
        query = rearrange(query, 'b h s d -> (b h) s d').contiguous()
        # key = attn.head_to_batch_dim(key)
        key = attn.separate_heads(key)
        key = rearrange(key, 'b s h d -> b h s d').contiguous()
        key[:, :, :video_length, :] = attn.rotary_emb.rotate_queries_or_keys(key[:, :, :video_length, :])
        key = rearrange(key, 'b h s d -> (b h) s d').contiguous()
...
class XFormersAttnProcessor:
    ...
    def __call__(
        ...
        if attention_mask is not None:
            attention_mask = attention_mask.unsqueeze(0).expand(batch_size, -1, -1) # attention_mask: 19 * 19
...
class AttnProcessor2_0:
    ...
    def __call__(
        ...
        if attention_mask is not None:
            attention_mask = attention_mask.unsqueeze(0).repeat_interleave(batch_size, dim=0)
            attention_mask = attn.prepare_attention_mask(attention_mask, sequence_length, batch_size)
        ...
        query = query.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2) # b h s d
        # query[:, :, :video_length, :] = attn.rotary_emb.rotate_queries_or_keys(query[:, :, :video_length, :])
