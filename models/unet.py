...
if __name__ == '__main__':
    ...
    pretrained_model_path = "/mnt/petrelfs/share_data/maxin/work/pretrained/stable-diffusion-2-1-base" # p cluster
    ...
    use_image_num = 6
    noisy_latents = torch.randn((3, 4, 16 + use_image_num, 32, 32)).to(device)
    bsz = noisy_latents.shape[0]
    timesteps = torch.randint(0, 1000, (bsz,)).to(device)
    timesteps = timesteps.long()
    encoder_hidden_states = torch.randn((bsz, 1 + use_image_num, 77, 1024)).to(device)
    pooled_prompt_embeds_batch = torch.randn((bsz, 1 + use_image_num, 1280)).to(device)
    
    original_size_batch = [[[336, 596], [740, 740], [439, 780], [325, 640]], [[336, 596], [953, 729], [366, 512], [330, 500]], [[336, 596], [480, 480], [390, 612], [480, 640]]]
    crops_coords_top_left_batch = [[[2, 74], [262, 5], [0, 75], [2, 93]], [[0, 19], [411, 0], [147, 0], [44, 3]], [[2, 62], [404, 18], [50, 19], [153, 2]]]
    target_size = [32, 32]
    add_time_ids_list = []
    ...
    for original_size, crops_coords_top_left in zip(original_size_batch, crops_coords_top_left_batch):
        add_time_ids = torch.cat([compute_time_ids(s, c) for s, c in zip(original_size, crops_coords_top_left)]).unsqueeze(0)
        add_time_ids_list.append(add_time_ids)

    # add_time_ids_batch = torch.concat(add_time_ids_list, dim=0).reshape(bsz * (1 + use_image_num), -1)
    add_time_ids_batch = torch.concat(add_time_ids_list, dim=0)

    unet_added_conditions = {"time_ids": add_time_ids_batch}
    unet_added_conditions.update({"text_embeds": pooled_prompt_embeds_batch})
