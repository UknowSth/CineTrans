...
try:
    ...
    from temporal_attention_v2 import Attention as TemporalAttentionv2
except:
    from .temporal_attention import TemporalAttention, CrossAttention
    from .temporal_attention_v2 import Attention as TemporalAttentionv2
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        #     hidden_states_video = self.attn_temp(norm_hidden_states_video, video_length=video_length, use_image_num=use_image_num) + hidden_states_video # v2
        #     # hidden_states_video = self.attn_temp(norm_hidden_states_video, video_length=video_length, use_image_num=3) + hidden_states_video # v2

        #     # hidden_states_video = self.attn_temp(norm_hidden_states_video, attention_mask=attention_mask_temp, 
        #     #                            video_length=video_length, use_image_num=use_image_num) + hidden_states_video # v1
        ...
        #     )
        #     hidden_states = self.attn_temp(norm_hidden_states, video_length=video_length) + hidden_states
        ...
        # process image and video with mask
        d = hidden_states.shape[1]
        hidden_states = rearrange(hidden_states, "(b f) d c -> (b d) f c", f=video_length + use_image_num).contiguous()
        norm_hidden_states = (
            self.norm_temp(hidden_states, timestep) if self.use_ada_layer_norm else self.norm_temp(hidden_states)
        )
        hidden_states = self.attn_temp(norm_hidden_states, video_length=video_length, use_image_num=use_image_num, shot_info=shot_info) + hidden_states
        # hidden_states = rearrange(hidden_states, "(b d) f c -> (b f) d c", d=d).contiguous()
